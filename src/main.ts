...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const animalTemplate = document.getElementById('animal-template')! as HTMLTemplateElement;
  const speciesTemplate = document.getElementById('species-template')! as HTMLTemplateElement;
  ...
  try { 
    ...
    speciesRow.addEventListener('click', (event) => {
      ...
      if (target.classList.contains('species-name')) {
        ...
        try {
          const filtered = animals.filter(animal => animal.species === selected);
    ...
    HTMLHelper.populateSpeciesTable(speciesRow, speciesTemplate, species);
  } catch (error) {
    console.error('Error fetching data:', error);
  }
});
